...
`connect_db()` 為 context manager，離開區塊時結束交易並關閉連線；sqlite3 連線自身的 `__exit__` 只負責 commit 與 rollback。連線帶有內部參考循環，未關閉時須等循環垃圾回收才釋放，併發下可能先耗盡檔案描述子。
