...
核心特性：
- **現金與整股記帳**：以現金加整股部位模擬資金曲線，零頭閒置形成真實的資金拖累
...
| `tests/test_rule.py` | 各規則的偏向讀值、交叉事件判定（含兩線貼合與假交叉）、宣告的欄位與暖身是否正確 |
| `tests/test_strategy.py` | 交叉方向到訊號的映射、反手訊號選擇、單輪狀態重置、RSI 門檻讀值 |
| `tests/test_composite.py` | 閘門否決與遲滯、進出場門檻、規則求值次數與可見視窗、訊號內容、桶內宣告合併 |
...
| `Gated` | 依角色組合：SMA60 趨勢作閘門，僅在其放行的方向接受反轉訊號 |
